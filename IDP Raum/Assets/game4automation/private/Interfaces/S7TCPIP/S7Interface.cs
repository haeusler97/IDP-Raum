...
namespace game4automation
{
    ...
    public class S7Interface : InterfaceThreadedBaseClass
    {
        ...
        public S7InterfaceSignal.S7TYPE GetS7Type(string s7type)
        {
        ...
        public InterfaceSignal.TYPE GetType(S7InterfaceSignal.S7TYPE s7type)
        {
            switch (s7type)
            {
        ...
        public InterfaceSignal.DIRECTION GetDirection(string name)
        {
        ...
        public static S7InterfaceSignal.S7TYPE GetTypeFromName(string name)
        {
        ...
        public override void ImportSignals(bool simstart)
        {
            NumberInputs = 0;
            NumberOutputs = 0;
            if (simstart)
            {
                ...
                foreach (var Signal in signals)
                {
                    ...
                    s7signal.DBNumber = GetNumberAfterPoint(Signal.Name);
        ...
        private void WriteS7()
        {
            ...
            while (toread > 0)
            {
                ...
                if (signal.Direction == InterfaceSignal.DIRECTION.INPUT)
                {
                    ...
                    if (pdu < NegotiatedPduLenght - ReduceMaxPduLenth && request < NegotiatedPduLenght - ReduceMaxPduLenth) // Add
                    {
                        if (signal.S7Type == S7InterfaceSignal.S7TYPE.BOOL)
        ...
        public override void CommunicationThreadUpdate()
        {
            ReadS7();
            WriteS7();
        }
        ...
        //! Method for things to do before the signal update
        public override void BeforeUpdateAllSignals()
        {
        }

        //! Method for things to do after the signal update
        public override void AfterUpdateAllSignals()
        {
        }


        //! Updates one signal in the communication thread
        public override void UpdateSignal(InterfaceSignal interfacesignal)
        {
            lock (InterfaceSignals)
            {
